...
2. TABLE-FORMAT FILTERING (format 1 only):
   ...
   A confirmation prompt appears before the check runs. Answering 'n' SKIPS
   the live check entirely (zero HTTP requests made) and proceeds directly
   with defaults: every row with a real Downloadable Link is included as-is,
   on the strength of the user's own assurance that the dataset contains no
   native Google Docs/Sheets/Slides exports. Use this when you've already
   verified your files yourself and don't want to wait on/risk the check.
...
def prompt_run_live_check(n: int) -> bool:
    print(
        f"\nThe Table format can verify {n} Downloadable Link(s) live — one real "
        f"HTTP (CURL-style) request per link, sequential, with a "
        f"{LINK_CHECK_TIMEOUT_SECONDS}s timeout and a {LINK_CHECK_DELAY_SECONDS}s "
        f"pause between requests — to catch any that return an HTTP error."
    )
    print(
        "  y = run the check now (recommended if you're not 100% sure any of "
        "these are native Google Docs/Sheets/Slides exports)"
    )
    print(
        "  n = skip it and proceed directly with defaults — every entry with a "
        "Downloadable Link is included as-is, no HTTP requests made at all "
        "(use this once you've already confirmed yourself that everything "
        "downloads fine)"
    )
    while True:
...
def filter_for_table_format(rows, do_live_check: bool = True):
    """Returns (kept, excluded, unverified_count, skipped_count) for the Table
    format.
    ...
    later by excluded_reason().

    Two distinct "didn't get a confirmed-good status" buckets are tracked
    separately so the final report doesn't conflate them:
      - unverified_count: a request WAS attempted but never got a response
        (timeout/network error) — kept on a best-effort basis.
      - skipped_count: do_live_check was False, so NO request was attempted
        at all for any row — every row with a real link is included purely
        on the user's own assurance, branched straight past the HTTP step.
    """
    checkable, no_link = [], []
    for row in rows:
    ...
    if not do_live_check:
        # Branch: skip the HTTP/CURL check entirely, proceed directly with
        # defaults — every checkable row is trusted and kept as-is, zero
        # network requests made.
        for row in checkable:
            row["_table_status"] = "default (unchecked)"
            kept.append(row)
        ...
        skipped_count = len(kept)
        print(
            f"\n✓ Live check skipped — proceeding directly with defaults: "
            f"{skipped_count} entr{'y' if skipped_count == 1 else 'ies'} with a "
            f"Downloadable Link {'is' if skipped_count == 1 else 'are'} included "
            f"in the Table format as-is, with NO HTTP requests made."
        )
        return kept, excluded, 0, skipped_count
    ...
    return kept, excluded, len(unverified), 0
...
def main():
    ...
    for format_choice in format_choices:
        ...
        if format_choice == FORMAT_TABLE:
            ...
            rows_for_format, excluded, unverified_count, skipped_count = filter_for_table_format(rows, do_check)
            table_unverified_total += unverified_count
            table_skipped_total += skipped_count
            if excluded:
                report_rows = [
    ...
    if table_skipped_total:
        print(
            f"ℹ Note: {table_skipped_total} Downloadable Link(s) were included in the "
            f"Table format WITHOUT a live check — you chose to skip verification and "
            f"proceed with defaults. If any of these turn out to be native Google "
            f"Docs/Sheets/Slides exports, their download button will break."
        )
